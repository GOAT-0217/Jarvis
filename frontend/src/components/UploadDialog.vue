<template>
  <el-dialog :model-value="visible" title="上传文档" width="480px" @update:model-value="$emit('update:visible', $event)" @open="loadFormData">
    <div class="upload-section">
      ...
      <div class="category-row">
        ...
        <template v-if="!showNewCat">
          <el-button @click="showNewCat = true" size="default" style="flex-shrink: 0">+ 新建</el-button>
        </template>
        <template v-else>
          <el-input v-model="newCatName" ref="newCatInput" placeholder="分类名" size="default" style="width: 120px; flex-shrink: 0" maxlength="20" @keydown.enter="createCategory" />
      ...
      <!-- 上传区域 -->
      <el-upload drag :http-request="customUpload" :show-file-list="false" accept=".pdf,.docx,.doc,.xlsx,.xls">
        <el-icon :size="40"><UploadFilled /></el-icon>
        <div style="margin-top: 8px">拖拽文件到此处或点击上传</div>
        <template #tip>
          <div style="margin-top: 8px; color: #6d6f78; font-size: 12px">支持 PDF、Word (.docx/.doc)、Excel (.xlsx/.xls)</div>
        </template>
      </el-upload>

      <div v-if="uploading" style="text-align: center; margin-top: 12px; color: #5eead4">正在上传…</div>
    </div>
  </el-dialog>
</template>
...
async function customUpload(options: any) {
  ...
  if (categoryId.value) formData.append('category_id', categoryId.value)
  if (tagIds.value.length) formData.append('tag_ids', tagIds.value.join(','))
  try {
    ...
    alert('上传成功，正在后台处理')
    emit('done')
  } catch (e: any) { alert('上传失败: ' + (e.message || '未知错误')) }
