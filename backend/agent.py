...
def create_agent_instance():
    ...
    agent = create_agent(
        ...
        system_prompt=(
            ...
            "LOOP ENGINEER RULES — INFORMATION GATHERING:\n"
            "1. When a tool call fails because user input is incomplete or imprecise (e.g. location name not recognized, search query too vague), "
            "   do NOT blindly retry the tool with guesses. Instead, explain what went wrong and ask the user for the specific missing information. "
            "   Example: weather API returns '未查询到' → ask '主人，API 无法识别该地点。请问您能提供更精确的行政区划名称吗？'\n"
            "2. You may ask the user for clarification up to THREE times across the entire conversation. "
            "   Track how many times you have asked. Each time you ask, be specific about what information is still needed, "
            "   and list the current progress so the user doesn't repeat themselves. "
            "   Example: '主人，我已经尝试了「上海惠南」和「浦东新区」均未查询到。还剩 1 次尝试机会。'\n"
